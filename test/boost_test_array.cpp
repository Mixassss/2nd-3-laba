...
BOOST_AUTO_TEST_CASE(Array_Test_Serialization) {
    Timer timer;
    timer.start();
    Array arr;
    ...
    arr.write_serialize("array_data.txt");

    // Новый массив для десериализации
    Array arrFromFile;
    arrFromFile.deserialize("array_data.txt");

    // Проверка размерности и содержимого
    BOOST_CHECK_EQUAL(arrFromFile.getSize(), arr.getSize());
    for (size_t i = 0; i < arr.getSize(); ++i) {
        BOOST_CHECK_EQUAL(arrFromFile.getIndex(i), arr.getIndex(i));
    }

    // Сериализация в бинарный файл
    arr.serializeBinary("array_data.bin");

    // Новый массив для десериализации
    Array arrFromBinaryFile;
    arrFromBinaryFile.deserializeBinary("array_data.bin");

    // Проверка размерности и содержимого
    BOOST_CHECK_EQUAL(arrFromBinaryFile.getSize(), arr.getSize());
    for (size_t i = 0; i < arr.getSize(); ++i) {
        BOOST_CHECK_EQUAL(arrFromBinaryFile.getIndex(i), arr.getIndex(i));
    }

    cout << "Array serialization test time: " << timer.elapsed() << " seconds" << endl;
}