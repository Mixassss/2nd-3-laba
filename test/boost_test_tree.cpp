...
BOOST_AUTO_TEST_CASE(TestSerializeToTextFile) {
    CompleteBinaryTree tree;
    tree.insert(5);
    tree.insert(2);
    tree.insert(9);
    ...
    tree.serializeToTextFile("tree.txt");

    // Создаем новую структуру для десериализации
    CompleteBinaryTree newTree;
    newTree.deserializeFromTextFile("tree.txt");

    // Проверка, что десериализованные данные корректны
...
BOOST_AUTO_TEST_CASE(TestSerializeToBinaryFile) {
    CompleteBinaryTree tree;
    tree.insert(5);
    tree.insert(2);
    tree.insert(9);
    ...
    tree.serializeToBinaryFile("tree.bin");

    // Создаем новую структуру для десериализации
    CompleteBinaryTree newTree;
    newTree.deserializeFromBinaryFile("tree.bin");
